...
This module provides a base :class:`ExperimentRunner` to standardise how
experiments are executed, logged and how their results are stored.
"""
...
class ExperimentRunner(abc.ABC):
    """Abstract base class for running an experiment in a structured way."""

    def __init__(self, experiment_name: str, base_output_dir: str = "experiments/results"):
        """
        Args:
            experiment_name: Descriptive name for the experiment (e.g. ``"fiqa_experiment"``).
            base_output_dir: Base directory where all results will be stored.
        """
    ...
    def _setup_run_environment(self):
        """Create a unique directory for the run and configure logging."""
    ...
    def _save_config(self, config: Any):
        """Save the experiment configuration to JSON."""
        if not self.run_dir:
            raise RuntimeError(
        ...
        if is_dataclass(config):
            ...
            self.logger.warning(
                "Configuration is neither a dataclass nor a dict; saving as string."
            )
    ...
    @abc.abstractmethod
    def run(self):
        """Abstract method containing the experiment logic."""
        pass
    ...
    def start(self, config: Any):
        ...
        # Configure the run environment and persist the configuration
        self._setup_run_environment()
        self._save_config(self.config)

        try:
            # Execute the experiment's main logic
            self.run()

...
class TrainerConfig(BaseModel):
    ...
    def validate_k_graph(cls, v):
        ...
        if v > 100:
            warnings.warn(
                f"k_graph={v} may be computationally expensive for large datasets",
                stacklevel=2,
            )
        return v
    ...
    def validate_regularization_weights(cls, v):
        ...
        if v > 10.0:
            warnings.warn(f"Large regularization weight {v} may dominate training", stacklevel=2)
        return v
    ...
    def validate_learning_rate(cls, v):
        ...
        if v > 0.1:
            warnings.warn(
                f"Learning rate {v} is unusually high, consider values < 0.01",
                stacklevel=2,
            )
        return v

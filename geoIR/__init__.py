...
def quick_experiment(
    ...
    **kwargs
    ...
    # ------------------------------------------------------------------
    # Load a tiny corpus and create synthetic triplets for demonstration
    # ------------------------------------------------------------------
    from .data.loader import load_corpus

    corpus = load_corpus(dataset)
    if len(corpus) < 3:
        raise ValueError("Dataset must contain at least three documents")

    triplets = [
        (corpus[i], corpus[(i + 1) % len(corpus)], corpus[(i + 2) % len(corpus)])
        for i in range(min(len(corpus) - 2, 10))
    ]

    trainer.train(triplets)

    index = encoder.build_index(corpus, k=k)

    query_emb = encoder.encode(corpus, is_query=True)
    from .eval.metrics import evaluate_retrieval

    qrels = {str(i): {str(i): 1} for i in range(len(corpus))}
    ndcg, recall = evaluate_retrieval(
        query_emb,
        index.embeddings,
        qrels,

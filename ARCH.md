...
| `research/beir_euclidean_vs_geo.py` | researchers | Experimental / research-only | `research/beir_euclidean_vs_geo.py:98` hardcodes `dataset_path = Path("/home/jd/.../datasets/fiqa")` and the BEIR download fallback is commented out, so the script only runs on one author's machine. | Parameterize the dataset location via `--data-dir`, restore or remove the download fallback explicitly, and document a small-scale recipe runnable from a clean checkout. |
...
- `research/beir_euclidean_vs_geo.py:98`: hardcoded dataset path and commented-out download fallback.
